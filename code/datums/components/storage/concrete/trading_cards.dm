...
/datum/storage/concrete/tcg/Initialize()
	. = ..()
	set_holdable(list(/obj/item/tcgcard))

/datum/storage/concrete/tcg/PostTransfer()
	. = ..()
	handle_empty_deck()

/datum/storage/concrete/tcg/remove_from_storage(atom/movable/AM, atom/new_location)
	. = ..()
	handle_empty_deck()

/datum/storage/concrete/tcg/show_to(mob/M)
	. = ..()
...
/datum/storage/concrete/tcg/close(mob/M)
	. = ..()
...
/datum/storage/concrete/tcg/mass_remove_from_storage(atom/target, list/things, datum/progressbar/progress, trigger_on_found)
	. = ..()
	if(!things.len)
		qdel(parent)

/datum/storage/concrete/tcg/proc/handle_empty_deck()
