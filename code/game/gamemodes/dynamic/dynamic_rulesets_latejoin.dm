...
/datum/dynamic_ruleset/latejoin/trim_candidates()
	for(var/mob/P in candidates)
		if(!P.client || !P.mind || is_unassigned_job(P.mind.assigned_role)) // Are they connected?
			...
			candidates.Remove(P)
		else if(P.mind.assigned_role.job_tag in restricted_roles) // Does their job allow for it?
			candidates.Remove(P)
		else if((exclusive_roles.len > 0) && !(P.mind.assigned_role.job_tag in exclusive_roles)) // Is the rule exclusive to their job?
			candidates.Remove(P)
