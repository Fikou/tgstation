...
/turf/storage_contents_dump_act(datum/storage/src_object, mob/user)
	. = ..()
	if(.)
		return
	...
	while (do_after(usr, 1 SECONDS, src, NONE, FALSE, CALLBACK(src_object, /datum/storage.proc/mass_remove_from_storage, src, things, progress)))
		stoplag(1)
