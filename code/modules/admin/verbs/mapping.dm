...
/client/proc/create_mapping_job_icons()
	...
	for(var/job in subtypesof(/datum/job))
		...
		switch(JB.job_tag)
			if("AI")
				final.Insert(icon('icons/mob/silicon/ai.dmi', "ai", SOUTH, 1), JB.job_tag)
			if("CYBORG")
				final.Insert(icon('icons/mob/silicon/robots.dmi', "robot", SOUTH, 1), JB.job_tag)
			else
