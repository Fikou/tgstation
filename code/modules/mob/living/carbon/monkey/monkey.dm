...
/mob/living/carbon/human/monkey/Initialize()
	. = ..()
	INVOKE_ASYNC(src, .proc/set_species, /datum/species/monkey)
...
// taken from /mob/living/carbon/human/interactive/
/mob/living/carbon/human/monkey/proc/IsDeadOrIncap()
	return HAS_TRAIT(src, TRAIT_INCAPACITATED) || HAS_TRAIT(src, TRAIT_HANDS_BLOCKED)


/mob/living/carbon/human/monkey/proc/battle_screech()
	if(next_battle_screech < world.time)
		emote(pick("roar","screech"))
		for(var/mob/living/carbon/human/monkey/M in view(7,src))
			M.next_battle_screech = world.time + battle_screech_cooldown

/mob/living/carbon/human/monkey/proc/equip_item(obj/item/I)
	if(I.loc == src)
		return TRUE
...
/mob/living/carbon/human/monkey/proc/pickup_and_wear(obj/item/clothing/C)
...
/mob/living/carbon/human/monkey/proc/should_target(mob/living/L)
	...
	if(aggressive && (!ismonkey(L) || prob(MONKEY_AGGRESSIVE_MVM_PROB)))
		return TRUE
...
/mob/living/carbon/human/monkey/proc/pickpocket(mob/M)
...
/mob/living/carbon/human/monkey/attack_paw(mob/living/L)
	if(L.a_intent == INTENT_HARM && prob(MONKEY_RETALIATE_HARM_PROB))
		retaliate(L)
	else if(L.a_intent == INTENT_DISARM && prob(MONKEY_RETALIATE_DISARM_PROB))
		retaliate(L)
...
/mob/living/carbon/human/monkey/attackby(obj/item/W, mob/user, params)
	..()
	if((W.force) && (!target) && (W.damtype != STAMINA) )
		retaliate(user)

/mob/living/carbon/human/monkey/bullet_act(obj/projectile/Proj)
...
/mob/living/carbon/human/monkey/hitby(atom/movable/AM, skipcatch = FALSE, hitpush = TRUE, blocked = FALSE, datum/thrownthing/throwingdatum)
...
/mob/living/carbon/human/monkey/proc/monkeyDrop(obj/item/A)
	if(A)
		dropItemToGround(A, TRUE)
		update_icons()

/mob/living/carbon/human/monkey/grabbedby(mob/living/carbon/user)
	. = ..()
