...
/mob/living/carbon/human/monkey/punpun/Initialize()
	Read_Memory()
	if(ancestor_name)
		real_name = ancestor_name
		if(ancestor_chain > 1)
			real_name += " \Roman[ancestor_chain]"
	else if(prob(10))
		real_name = pick(list("Professor Bobo", "Deempisi's Revenge", "Furious George", "King Louie", "Dr. Zaius", "Jimmy Rustles", "Dinner", "Lanky"))
		if(real_name == "Furious George")
